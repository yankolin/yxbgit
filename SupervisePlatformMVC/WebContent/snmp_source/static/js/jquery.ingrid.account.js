...
jQuery.fn.ingridaccount = function(o){
	...
	var cfg = {
		...
		colWidths: [50,100,200,100,100,200,10],			// width of each column
		...
		gridtheadthClass:'grid_thead_th',       //set theard th in the center
		/* sorting */
		sorting: false,
	...
	if (cfg.paging) {
		...
		// start page button
		var pb1	= jQuery('<a href="javascript:void(0)">&laquo;</a>').addClass(cfg.pageStartClass).click(function(){
			p.setPage(1);
		});
	
		// prev page button
		var pb2 = jQuery('<a href="javascript:void(0)">&lt;</a>').addClass(cfg.pagePrevClass).click(function(){
			var _p = p.getPage();																															
			if (_p > 1) {
				_p--;
				p.setPage(_p);
		...
		var pb3 = jQuery('<a href="javascript:void(0)">&gt;</a>').addClass(cfg.pageNextClass).click(function(){
			var _p = p.getPage(); _p++;
			if (totp) {
		...
		// page field & form
		var pfld  = jQuery('<input type="text" value="' + cfg.pageNumber + '" disabled="disabled" />').addClass(cfg.pageInputClass);
		var pinfo = jQuery('<div />')
			.addClass(cfg.pageInfoClass)
			.append(pfld);
		var pform = jQuery('<form  method="get"></form>').append(pinfo).submit(function(){
		...
		if (cfg.totalRecords > 0) {
			...
			pb4	= jQuery('<a href="javascript:void(0)">&raquo;</a>').addClass(cfg.pageEndClass).click(function(){
				var _p = p.getPage(); _p++;
				if (totp) {
		...
		initStylesAndWidths : function() {
			...
			this.getRows().each(function(r){
				...
				if (cfg.rowSelection == true) {
					jQuery(this).find("button[id$='u_alter']").click(function() {
						if (cfg.onRowSelect)
								cfg.onRowSelect( $(this).parents("tr"), (jQuery(this).attr( cfg.rowSelectedAttr ) == 'true'),1 );
						});
					jQuery(this).find("button[id$='u_delete']").click(function() {
						if (cfg.onRowSelect)
								cfg.onRowSelect( $(this).parents("tr"), (jQuery(this).attr( cfg.rowSelectedAttr ) == 'true'),2 );
						});
					jQuery(this).find("button[id$='u_resetpwd']").click(function() {
						if (cfg.onRowSelect)
								cfg.onRowSelect( $(this).parents("tr"), (jQuery(this).attr( cfg.rowSelectedAttr ) == 'true'),3 );
						});
